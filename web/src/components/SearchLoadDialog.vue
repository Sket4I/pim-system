    ...
    <v-card>
      ...
      <v-card-text>
        <v-container>
          <v-row>
            <v-col cols="7">
              <v-text-field color="black" v-model="searchRef" :label="$t('Main.Search')" flat hide-details clearable clear-icon="mdi-close-circle-outline"></v-text-field>
              <v-treeview color="black" dense activatable :items="itemsRef" :open="openItemsRef" :search="searchRef" @update:active="activeChanged" style="max-height: 300px" class="overflow-y-auto">
            ...
            <v-col cols="5">
              <v-form ref="formRef" lazy-validation class="ml-7"  v-if="selectedRef">
                <v-text-field color="black" readonly v-model="selectedRef.identifier" :disabled="selectedRef.id !== 0" :label="$t('SearchSaveDialog.Identifier')" required></v-text-field>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="black" :disabled="!selectedRef || selectedRef.user !== currentUserRef.login" text @click="removeSearch">{{ $t('Remove') }}</v-btn>
        <v-btn color="black" text @click="dialogRef = false">{{ $t('Cancel') }}</v-btn>
        <v-btn color="black" :disabled="!selectedRef" text @click="selected">{{ $t('Select') }}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
