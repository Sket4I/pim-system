<template>
  ...
  <v-col cols="12">
    ...
    <v-list nav dense class="ma-0" v-if="selectedRef && !selectedRef.extended">
      <v-list-item-group v-model="selectedFilterRef" color="black">
        <v-list-item v-for="(filter, i) in selectedRef.filters" :key="i" :value="i" :three-line="filter.type === 'attr'">
          ...
          <v-list-item-content>
            <v-container class="pa-0">
              <v-row no-gutters>
                <v-col cols="12">
                  <v-autocomplete color="black" dense v-model="filter.attr" :items="fieldsSelection" :label="$t('Search.Filter.Attribute.Attr')"></v-autocomplete>
                </v-col>
              </v-row>

              <v-row no-gutters v-if="filter.attr !== '#level#' && filter.attr !== 'collectionId'">
                <v-col cols="12">
                  <v-select item-color="black" color="black" dense v-model="filter.operation" :items="operationSelection" :label="$t('Search.Filter.Attribute.Operation')"></v-select>
                </v-col>
              </v-row>
              <v-row no-gutters v-if="filter.attr && (!filter.attr.endsWith('#status') || filter.attr === 'attr#status')">
                <v-col cols="12">
                  <template v-if="filter.attr === '#level#'">
                    <v-text-field color="black" dense readonly v-model="filter.value" :label="$t('Search.Filter.Attribute.Value')" required append-outer-icon="mdi-form-select" @click:append-outer="itemSelectionDialogRef.showDialog(filter)"></v-text-field>
                  </template>
                  <template v-if="filter.attr === 'collectionId'">
                    <v-text-field color="black" dense readonly v-model="filter.value" :label="$t('Search.Filter.Attribute.Value')" required append-outer-icon="mdi-form-select" @click:append-outer="collSelectionDialogRef.showDialog(false, filter)"></v-text-field>
                  </template>
                  <v-autocomplete color="black" v-if="filter.attr && filter.attr !== '#level#' && filter.attr !== 'collectionId' && lovsMapRef[filter.attr]" dense v-model="filter.value" :items="getLovItems(filter)" :label="$t('Search.Filter.Attribute.Value')"></v-autocomplete>
                  <v-text-field color="black" v-if="(filter.operation !== 10 && filter.operation !== 16 && filter.operation !== 17) && filter.attr && filter.attr !== '#level#' && filter.attr !== 'collectionId' && filter.attr !== 'typeIdentifier' && !getDateType(filter) && !lovsMapRef[filter.attr]" dense v-model="filter.value" :label="$t('Search.Filter.Attribute.Value')" required></v-text-field>
                  <v-text-field color="black" v-if="(filter.operation !== 10 && filter.operation !== 16 && filter.operation !== 17) && filter.attr && filter.attr === 'typeIdentifier' && !lovsMapRef[filter.attr]" dense v-model="filter.value" :label="$t('Search.Filter.Attribute.Value')" required append-outer-icon="mdi-file-document-edit-outline" @click:append-outer="typeSelectionDialogRef.showDialog(filter)"></v-text-field>
                  ...
                  <v-textarea color="black" v-if="filter.operation === 10 && filter.attr && filter.attr !== '#level#' && filter.attr !== 'collectionId' && !lovsMapRef[filter.attr]" dense v-model="filter.value" :label="$t('Search.Filter.Attribute.Value')" required></v-textarea>
                </v-col>
              </v-row>
              <v-row no-gutters v-if="filter.attr && filter.attr.endsWith('#status') && filter.attr !== 'attr#status'">
                <v-col cols="12">
                  <v-select item-color="black" color="black" dense v-model="filter.value" :items="statusSelection" :label="$t('ColumnsSelection.ChannelStatus')"></v-select>
                </v-col>
              </v-row>
    ...
    <v-textarea color="black" v-if="selectedRef && selectedRef.extended" class="ml-3 mr-3" v-model="extendedSearchRef" :label="$t('Search.Extended.Label')"></v-textarea>
</v-col>
<v-col cols="4" class="d-inline-flex justify-end align-center">
  <v-select item-color="black" color="black" v-if="selectedRef && !selectedRef.extended && selectedRef.filters && selectedRef.filters.length > 1" class="ml-5" dense v-model="selectedRef.orAnd" :items="orAndSelection"></v-select>
</v-col>
...
export default {
  ...
  setup (props, context) {
    ...
    async function searchSelected (selected) {
      ...
      searchEntityRef.value = selected.entity ? selected.entity : 'NOMEN'
    ...
    function search () {
      localStorage.setItem('last_nomen_search', JSON.stringify(selectedRef.value))
      if (selectedRef.value.extended) {
        try {
          selectedRef.value.whereClause = JSON.parse(extendedSearchRef.value)
          searchEntityRef.value = 'NOMEN'
        ...
        const where = {}
        where[orAndOperation] = [{ typeIdentifier: 'item' }]

        const brandWhere = {}
        brandWhere[orAndOperation] = []
          ...
          if (filter.attr) {
            ...
            if (filter.attr.startsWith('channel#')) {
              ...
              if (idx === -1) {
                ...
                if (attr === 'aStranaProichozhdeniya' || attr === 'aKategoriiniiMenedzher' || attr === 'aStranaDizaina' || attr === 'PDManager') brandAttr = true
            ...
            if (!brandAttr) {
              ...
              brandWhere[orAndOperation].push(data)
            }
          }
        })
        searchEntityRef.value = 'NOMEN'
        ...
        if (brandWhere[orAndOperation].length > 0) {
          where.include = [{ as: 'targetRelation', required: true, where: { relationIdentifier: 'relBrand' }, include: [{ as: 'sourceItem', required: true, where: brandWhere }] }]
        }
    ...
    onMounted(() => {
      ...
      Promise.all([loadAllTypes(), loadAllLanguages(), loadAllAttributes(), loadAllChannels()]).then(async () => {
        ...
        if (!selectedRef.value) {
          selectedRef.value = { identifier: '', entity: 'NOMEN', name: name, filters: [], whereClause: {}, extended: false, public: false, orAnd: 1 }
        }
        ...
        if (id) {
          ...
          if (tst) {
            ...
            const tst2 = localStorage.getItem('last_nomen_search')
