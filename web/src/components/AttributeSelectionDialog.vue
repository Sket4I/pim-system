  ...
  <v-dialog v-model="selectionDialogRef" persistent max-width="600px">
    <v-card>
        ...
        <v-container>
          <v-row>
            <v-col cols="12" class="pa-0 overflow-y-auto" style="max-height: 70vh">
              ...
              <v-treeview color="black" dense activatable hoverable :items="groupsFiltered" @update:active="activeChanged" :active="activeRef">
      ...
      <v-card-actions>
        ...
        <v-btn color="black" text @click="selected" :disabled="!selectedAttrRef" >{{ $t('Select') }}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
