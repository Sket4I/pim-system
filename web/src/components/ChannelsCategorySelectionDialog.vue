  ...
  <v-dialog v-model="selectionDialogRef" persistent max-width="90%">
    <v-card>
        ...
        <v-container>
          <v-row>
            <v-col cols="12">
              ...
              <v-treeview color="black" :search="searchRef" dense activatable hoverable :items="itemsRef" :active.sync="activeRef" :open="openRef">
      ...
      <v-card-actions>
        ...
        <v-btn color="black" text @click="selected" :disabled='activeRef.length===0 || activeRef[0].startsWith("CHAN")'>{{ $t('Select') }}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
