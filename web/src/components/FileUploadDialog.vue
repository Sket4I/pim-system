<template>
  <v-dialog v-model="dialogRef" persistent width="80%">
    <v-card>
          ...
          <v-form ref="formRef" lazy-validation class="ml-7">
            <v-row>
              <v-col cols="12">
                  ...
                  <v-select item-color="black" color="black" v-model="relationRef" :items="relationsWithFiles" :label="$t('FileUploadDialog.FileType')"></v-select>
                  <div class="d-inline-flex align-center">
                    ...
                    <v-btn color="black" text @click="itemSelectionDialogRef.showDialog()">{{ $t('FileUploadDialog.SelectParent.Button') }}</v-btn>
                  </div>
              </v-col>
            </v-row>
            <v-row>
                  ...
                  <v-expansion-panel :disabled="fileRef && fileRef.length > 1">
                    ...
                    <v-expansion-panel-content>
                       <v-container class="pa-0">
                        <v-row no-gutters>
                          <v-text-field color="black" class="pt-4 pb-0 pr-5 pl-5" v-model="identifierRef" :label="$t('FileUploadDialog.Identifier')" required></v-text-field>
                        </v-row>
                        <v-row no-gutters>
                          <v-text-field color="black" class="pt-4 pb-0 pr-5 pl-5" v-model="nameRef" :label="$t('FileUploadDialog.Name')" required></v-text-field>
                        </v-row>
                       </v-container>
                    </v-expansion-panel-content>
                  </v-expansion-panel>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="black" text @click="closeDialog">{{ $t('Cancel') }}</v-btn>
        <v-btn color="black" text @click="upload" :disabled="!fileRef || fileRef.length === 0 || !relationRef || !selectedParentRef">{{ $t('ItemView.Upload') }}</v-btn>
      </v-card-actions>
    </v-card>
