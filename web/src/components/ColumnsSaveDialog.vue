<template>
  <v-dialog v-model="dialogRef" persistent max-width="600px">
    <v-card>
        ...
        <v-container>
          <v-row>
            ...
            <v-list nav dense style="max-height: 300px" class="overflow-y-auto">
              <v-list-item-group v-model="indexlInListRef" color="black">
            ...
            <v-col cols="6">
              <v-form ref="formRef" lazy-validation class="ml-7" v-if="selectedRef">
                <div class="d-inline-flex align-center">
                  <v-text-field color="black" style="min-width: 100%" v-model="selectedRef.identifier" :disabled="selectedRef.id !== -1" :error-messages="identifierErrors" :rules="identifierRules" :label="$t('ColumnsSaveDialog.Identifier')" required></v-text-field>
                </div>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="black" text @click="addColumns">{{ $t('Add') }}</v-btn>
        <v-btn color="black" text :disabled="!selectedRef || selectedRef.id === -1" @click="removeColumnsHandler">{{ $t('Remove') }}</v-btn>
        <v-btn color="black" text :disabled="!selectedRef" @click="saveColumnsHandler">{{ $t('Save') }}</v-btn>
