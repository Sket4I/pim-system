            ...
            <v-card class="elevation-12">
              ...
              <v-card-text>
                <v-form>
                  ...
                  <v-select item-color="black" color="black" prepend-icon="mdi-translate" v-if="languageSelect" v-model="i18n.locale" :items="localeSelection" :label="$t('Login.Language')"></v-select>
                </v-form>
              </v-card-text>
              <v-card-actions>
                <v-spacer />
                <v-btn v-if="!disableWebLogin" color="black" dark :disabled="!login || !password" @click="signIn(login, password, pathAfterLogin)">{{ $t('Login.Login') }}</v-btn>
                <template v-if="authProvidersRef.length > 0">
                  <v-btn v-for="(auth, i) in authProvidersRef" :key="i" color="black" @click="signInThrough(auth)">{{ $t('Login.Through') + auth.name }}</v-btn>
                </template>
              </v-card-actions>
            </v-card>
          </v-col>
