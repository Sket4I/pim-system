              ...
              <th class="text-left">
                <v-tooltip top>
                  <template v-slot:activator="{ on }">
                    <v-btn v-on="on" class="pa-0" icon color="black" @click="addValue"><v-icon dark>mdi-plus</v-icon></v-btn>
                  </template>
                  <span>{{ $t('Add') }}</span>
                </v-tooltip>
              </th>
          ...
          <tbody>
            <tr v-for="(elem, j) in attrOptions" :key="j">
              ...
              <td class="pa-1 pr-10" style="width:220px;">
                <v-select item-color="black" color="black" v-model="elem.type" :items="typeSelection" :label="$t('OptionsTable.Type')" @change="changeType(elem)" />
              </td>
              <td class="pa-1" colspan="2">
                ...
                <v-btn class="pa-0" icon color="black" @click="removeValue(j)"><v-icon dark>mdi-close-circle-outline</v-icon></v-btn>
                <v-btn class="pa-0" icon color="black" @click="addItems(j)" v-if="elem.type !== 1"><v-icon>mdi-file-document-edit-outline</v-icon></v-btn>
              </td>
            </tr>
          </tbody>
        </template>
