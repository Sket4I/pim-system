  ...
  <v-container v-if="canViewConfigRef" style="background-color:white">
    <v-row no-gutters>
      <v-col cols="5">
        ...
        <v-text-field color="black" v-model="searchRef" :label="$t('Filter')" flat hide-details clearable clear-icon="mdi-close-circle-outline" class="ml-5 mr-5"></v-text-field>
        <v-treeview color="black" :search="searchRef" :filter="filter" dense activatable hoverable :items="typesTree" @update:active="activeChanged" :active="activeRef" :open="openRef">
        ...
        <v-form ref="formRef" lazy-validation class="ml-7" v-if="selectedRef.id != -1">
          <div v-if="selectedRef.link === 0">
            <div class="d-inline-flex align-center">
              <v-text-field color="black" style="min-width: 100%" v-model="selectedRef.identifier" :disabled="selectedRef.internalId !== 0" :rules="identifierRules" :label="$t('Config.Types.ObjectType.Identifier')" required></v-text-field>
            ...
            <v-text-field color="black" v-model="selectedRef.icon" :label="$t('Config.Types.ObjectType.Icon')" required></v-text-field>
            <v-text-field color="black" v-model="selectedRef.iconColor" :label="$t('Config.Types.ObjectType.IconColor')" required></v-text-field>
            <v-checkbox v-model="selectedRef.file" :label="$t('Config.Types.ObjectType.File')"></v-checkbox>
              ...
              <v-card class="mb-5 mt-2">
                ...
                <v-list dense class="pt-0 pb-0">
                  <v-list-item-group v-model="relSelectedRef" color="black">
          ...
          <div v-else>
            ...
            <div class="d-inline-flex align-center">
              <v-text-field color="black"  class="mt-4" v-model="linkParent.identifier" readonly :label="$t('Config.Types.ObjectType.Identifier')" required></v-text-field>
              <SystemInformation :data="selectedRef"></SystemInformation>
            </div>
            ...
            <v-text-field color="black" v-model="linkParent.icon" readonly :label="$t('Config.Types.ObjectType.Icon')" required></v-text-field>
            <v-text-field color="black" v-model="linkParent.iconColor" readonly :label="$t('Config.Types.ObjectType.IconColor')" required></v-text-field>
