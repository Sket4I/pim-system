  ...
  <v-dialog v-model="selectionDialogRef" persistent max-width="800px">
    ...
    <v-list nav dense expand class="pa-5 ma-0">
      ...
      <v-row class="pt-0 pb-2 ma-0">
        <v-text-field color="black" v-model="searchRef" @input="clearSelection" :label="$t('Filter')" flat hide-details clearable
      ...
      <v-list-group :value="true" no-action>
        ...
        <v-list-item-group v-model="collectionRefMain" color="black">
      ...
      <v-list-group>
        ...
        <v-list-item-group v-model="collectionRefOther" color="black">
      ...
      <v-card-actions>
        ...
        <v-btn color="black" v-if="!deleteRef && !searchCollRef" :disabled="disableButton()" text @click="submit">{{ $t('Submit') }}</v-btn>
        <v-btn color="black" v-if="deleteRef && !searchCollRef" :disabled="disableButton()" text @click="delFromColl">{{ $t('Collections.DeleteFromCollection') }}</v-btn>
        <v-btn color="black" v-if="searchCollRef" :disabled="disableButton()" text @click="selectedColl">{{ $t('Select') }}</v-btn>
        </v-card-actions>
    </v-list>
  </v-dialog>
</template>
