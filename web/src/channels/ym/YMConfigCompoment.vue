    ...
    <v-tabs-items v-model="tabRef">
      ...
      <v-tab-item>
        ...
        <div class="d-inline-flex align-center">
          ...
          <v-btn color="black" v-if="!readonly" text @click="itemSelectionDialogRef.showDialog()">Выбрать</v-btn>
        </div>
      ...
      <v-tab-item>
        <div class="ml-5">
        <v-text-field color="black" v-if="channel" v-model="channel.config.file" :readonly="readonly" label="Имя генерируемого файла" required></v-text-field>
        <v-text-field color="black" v-if="channel" v-model="channel.config.mime" :readonly="readonly" label="Mime тип генерируемого файла" required></v-text-field>
        <v-text-field color="black" v-if="channel" v-model="channel.config.extCmd" :readonly="readonly" label="Запустить эту программу после генерации файла (если надо). Файл будет передан как {file} параметр." required></v-text-field>
        </div>
      </v-tab-item>
    </v-tabs-items>
