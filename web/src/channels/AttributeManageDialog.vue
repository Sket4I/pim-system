    ...
    <v-dialog v-model="dialogRef" persistent width="80%">
      <v-card v-if="attrRef">
            ...
            <v-row>
              <v-col cols="12">
                <v-form ref="formRef" lazy-validation>
                  <v-autocomplete color="black" chips deletable-chips :multiple="initialGroups && initialGroups.length > 0" :readonly="initialGroups && initialGroups.length > 0" :clearable="!initialGroups" item-text="text" item-value='id' v-model="groupRef" :items="groupsSelectionRef" :label="$t('AttributeManageDialog.Group')"/>
        ...
        <v-card-actions>
          ...
          <v-btn color="black" text @click="manage" :disabled="!groupRef || groupRef.length === 0">{{ $t('Execute') }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-row>
