    ...
    <v-row no-gutters>
      <v-col cols="12">
        <v-card outlined>
          <v-card-title>
            <v-row dense>
              <v-col cols="3" class="pl-3 mt-2">
                <v-text-field color="black" v-if="loadedRef" v-model="collectionNameRef" :readonly="!readonlyUser()" :label="$t('Collections.Name')"></v-text-field>
              </v-col>
          ...
          <v-card-actions>
            <v-btn v-if="readonlyUser()" :color="itemChangedRef ? 'black' : ''" depressed :text="!itemChangedRef" @click="save" v-text="$t('Save')"></v-btn>
