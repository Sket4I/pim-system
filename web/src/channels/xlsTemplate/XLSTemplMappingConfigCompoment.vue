            ...
            <div v-if="categoryIdRef">
                ...
                <v-row>
                    <v-col cols="11">
                        ...
                        <div v-if="sheetIdRef">
                            <v-text-field color="black" type="number" :readonly="readonly" v-model="sheetRef.index" :label="$t('MappingConfigComponent.SheetIndex')" required></v-text-field>
                            <v-text-field color="black" type="number" :readonly="readonly" v-model="sheetRef.rowStartFrom" :label="$t('MappingConfigComponent.SheetRowStartFrom')" required></v-text-field>
                                ...
                                <template v-slot:default>
                                    ...
                                    <tbody>
                                        <tr v-for="(elem, i) in sheetRef.colunms" :key="i">
                                            <td class="pa-1">
                                                <v-text-field color="black" v-model="elem.column" dense :readonly="readonly" class="ml-3 mr-3 d-inline-flex" />
                                            </td>
                                            <td class="pa-1">
                                                <v-autocomplete color="black" dense :readonly="readonly" v-model="elem.attrIdent" :items="allAttributes"></v-autocomplete>
                                            </td>
                                            <td class="pa-1">
                                                <v-text-field color="black" v-model="elem.expr" dense :readonly="readonly" class="ml-3 mr-3 d-inline-flex" :prepend-icon="''" @click:prepend="showOptions(sheetRef.colunms[i])" append-outer-icon="mdi-message-outline" @click:append-outer="showExpression(sheetRef.colunms[i])" />
                                            </td>
                                            <td>
          ...
          <v-card>
            ...
            <v-card-text>
              <v-container>
                <v-row>
                  <v-col cols="12">
                    <v-text-field color="black" v-model="newSheetNameRef" :label="$t('MappingConfigComponent.SheetName')" required></v-text-field>
                  </v-col>
                </v-row>
              </v-container>
            </v-card-text>
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="black" text @click="dialogSheetRef = false">{{ $t('Cancel') }}</v-btn>
              <v-btn color="black" text :disabled="!newSheetNameRef" @click="addSheet">{{ $t('Select') }}</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
      </v-row>
        ...
        <v-row justify="center" v-if="exprAttrRef">
            ...
            <v-card>
                ...
                <v-container>
                    <v-row>
                    <v-col cols="12">
                        <v-textarea color="black" :rows="15" :readonly="readonly" v-model="exprAttrRef.expr"></v-textarea>
                    </v-col>
                    </v-row>
                </v-container>
                </v-card-text>
                <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="black" text @click="exprDialogRef = false">{{ $t('Close') }}</v-btn>
                </v-card-actions>
            </v-card>
            </v-dialog>
        </v-row>
      ...
      <v-row justify="center" v-if="optAttrRef">
        <v-dialog v-model="optDialogRef" persistent max-width="90%">
          <v-card>
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="black" text @click="optDialogRef = false">{{ $t('Close') }}</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
      </v-row>
