    ...
    <v-dialog v-model="dialogRef" persistent width="50%">
      <v-card v-if="newCollectionRef">
            ...
            <v-row>
              <v-col cols="12">
                <v-form ref="formRef" lazy-validation>
                  <v-text-field color="black" v-model="newCollectionRef.identifier" :error-messages="identifierErrors" :rules="identifierRules" :label="$t('CollectionCreationDialog.Identifier')" required></v-text-field>
                  <v-text-field color="black" v-model="newCollectionRef.name[currentLanguage.identifier]" :rules="nameRules" :label="$t('CollectionCreationDialog.Name')" required></v-text-field>
        ...
        <v-card-actions>
          ...
          <v-btn color="black" text @click="create">{{ $t('Create') }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-row>
