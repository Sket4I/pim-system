<template>
  <v-dialog v-model="typeSelectionDialogRef" persistent width="80%">
    <v-card>
      <v-card-title>
        ...
        <v-container>
          <v-row>
            <v-col cols="12">
              ...
              <v-treeview color="black" v-if="typeSelectionDialogRef" :search="searchRef" :filter="filter" dense selectable selection-type="independent" v-model="selectedItemsRef" @input="onSelect" hoverable :items="filteredTree">
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="black" text @click="typeSelectionDialogRef = false">{{ $t('Cancel') }}</v-btn>
