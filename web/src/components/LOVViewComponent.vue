  ...
  <div v-if="lov">
    <v-form ref="formRef" lazy-validation class="ml-7" v-if="lov.id != -1">
      <div class="d-inline-flex align-center">
        <v-text-field color="black" style="min-width: 100%" v-model="lov.identifier" :disabled="lov.internalId !== 0"
      ...
      <v-data-table :headers="headers" :items="filteredValues" dense fixed-header height="60vh" :page.sync="currentPage"
        ...
        <template v-slot:top>
          <div class="d-flex align-items-center justify-content-between">
            <v-text-field color="black" type="text" class="pa-0" v-model="search" append-icon="mdi-magnify" :label="$t('Search')"
              single-line></v-text-field>
            <div class="d-flex align-items-center">
              <v-tooltip top v-if="canEditConfig" class="ml-4">
                <template v-slot:activator="{ on }">
                  <v-btn v-on="on" class="pa-0" icon color="black" @click="addValue">
              ...
              <v-tooltip top class="ml-4">
                <template v-slot:activator="{ on }">
                  <v-btn v-on="on" class="pa-0" icon color="black" @click="exportData">
              ...
              <v-tooltip top class="ml-4">
                <template v-if="canEditConfig" v-slot:activator="{ on }">
                  <v-btn v-on="on" class="pa-0" icon color="black" @click="importData">
        ...
        <template v-slot:item="{ item }">
          <tr>
            ...
            <td class="pa-1">
              ...
              <v-btn class="pa-0" icon color="black" @click="removeValue(item.id)">
      ...
      <v-row justify="center">
        <v-dialog v-model="dialogRef" persistent max-width="600px">
          <v-card>
                  ...
                  <v-col cols="12">
                    <v-card class="mb-5">
                      ...
                      <v-list dense class="pt-0 pb-0">
                        <v-list-item-group v-model="visibleSelectedRef" color="black">
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="black" text @click="dialogClose">{{ $t('Close') }}</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
      </v-row>
      <v-row justify="center">
        <v-dialog v-model="dialogAttrRef" persistent max-width="600px">
          <v-card>
                ...
                <v-row>
                  <v-col cols="12">
                    <v-card class="mb-5">
                      ...
                      <v-divider></v-divider>
                      <v-list dense class="pt-0 pb-0">
                        <v-list-item-group v-model="attrSelectedRef" color="black">
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="black" text @click="dialogAttrClose">{{ $t('Close') }}</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
      </v-row>
          ...
          <v-card>
            ...
            <v-card-text>
              <v-container>
                <v-row>
                  <v-col cols="12">
                    <v-textarea color="black" rows="7" v-model="importDataRef"></v-textarea>
                  </v-col>
                </v-row>
              </v-container>
            </v-card-text>
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="black" text @click="importDialogRef = false">{{ $t('Cancel') }}</v-btn>
              <v-btn color="black" text @click="processImport">{{ $t('Execute') }}</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
      </v-row>
