      ...
      <v-card v-if="newItemRef">
        ...
        <v-card-text>
          <v-container>
            <v-row>
              <v-col cols="12">
                <v-autocomplete color="black" v-model="identifierSelectedRef" label="Категория" :items="selectRef" item-text="text" item-value="id"></v-autocomplete>
              </v-col>
            </v-row>
          </v-container>
        </v-card-text>
        <v-card-actions>
          ...
          <v-btn color="black" text :disabled="!identifierSelectedRef" @click="create">{{$t('Create')}}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-row>
