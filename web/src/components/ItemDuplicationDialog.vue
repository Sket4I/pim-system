    ...
    <v-dialog v-model="dialogRef" persistent width="50%">
      <v-card v-if="newItemRef">
          ...
          <v-container>
            <v-row>
              <v-col cols="12">
                ...
                <v-text-field color="black" v-model="newItemRef.identifier" :error-messages="identifierErrors" :rules="identifierRules" :label="$t('ItemCreationDialog.Identifier')" required></v-text-field>
                <v-text-field color="black" v-model="newItemRef.name[currentLanguage.identifier]" :rules="nameRules" :label="$t('ItemCreationDialog.Name')" required></v-text-field>
                    ...
                    <div><div class="d-inline-flex align-center">
                      ...
                      <v-btn color="black" text @click="typeSelectionDialogRef.showDialog()">{{ $t('ItemDuplicationDialog.AnotherType') }}</v-btn>
                    </div></div>
        ...
        <v-card-actions>
          ...
          <v-btn color="black" text @click="duplicate">{{ $t('Duplicate') }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
