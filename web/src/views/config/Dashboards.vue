      ...
      <v-col cols="12">
        <v-form ref="formRef" lazy-validation class="ml-7" v-if="selectedRef.id != -1">
          <div class="d-inline-flex align-center">
            <v-text-field color="black" style="min-width: 100%" v-model="selectedRef.identifier"  :disabled="selectedRef.internalId !== 0" :rules="identifierRules" :label="$t('Config.Dashboards.Identifier')" required></v-text-field>
          ...
          <v-textarea color="black" v-model="selectedRef.usersStr" :label="$t('Config.Dashboards.Users')" rows="2"></v-textarea>
          ...
          <v-container>
            <v-row no-gutters>
              <v-col cols="3">
                ...
                <v-list nav dense>
                  <v-list-item-group v-model="componentRef" color="black">
              ...
              <v-col cols="9" v-if="selectedRef.components[componentRef]">
                <v-text-field color="black" v-model="selectedRef.components[componentRef].title" :label="$t('Config.Dashboards.Components.Title')" required></v-text-field>
                <v-text-field color="black" v-model="selectedRef.components[componentRef].width" type="number" :label="$t('Config.Dashboards.Components.Width')" required></v-text-field>

                <v-select item-color="black" color="black" v-model="selectedRef.components[componentRef].type" :items="typeSelection" :label="$t('Config.Dashboards.Components.Type')"></v-select>

                <template v-if="selectedRef.components[componentRef].type !== 3">
                  <v-select item-color="black" color="black" v-model="selectedRef.components[componentRef].chart" :items="chartSelection" :label="$t('Config.Dashboards.Components.ChartType')"></v-select>

                  <v-autocomplete color="black" v-if="selectedRef.components[componentRef].type === 1" v-model="selectedRef.components[componentRef].groupBy" :items="availableAttributes" :label="$t('Config.Dashboards.Components.Attribute')"></v-autocomplete>
                  <v-textarea color="black" v-if="selectedRef.components[componentRef].type === 1" v-model="selectedRef.components[componentRef].groupWhere" :label="$t('Config.Dashboards.Components.Where')" rows="3"></v-textarea>
                  ...
                  <template v-if="selectedRef.components[componentRef].type === 2">
                    ...
                    <v-list nav dense>
                      <v-list-item-group v-model="queryRef" color="black">
                    ...
                    <template v-if="queryRef != null">
                      <v-text-field color="black" v-model="selectedRef.components[componentRef].queries[queryRef].label" :label="$t('Config.Dashboards.Components.Label')" required></v-text-field>
                      <v-textarea color="black" v-model="selectedRef.components[componentRef].queries[queryRef].where" :label="$t('Config.Dashboards.Components.Where')" rows="3"></v-textarea>
                    </template>
                  </template>
                </template>
                <template v-else>
                  <v-text-field color="black" v-model="selectedRef.components[componentRef].url" :label="$t('Config.Dashboards.Components.Url')" required></v-text-field>
