    ...
    <v-card>
      ...
      <v-card-text>
        <v-container>
          <v-row>
            <v-col cols="12">
              <v-text-field color="black" v-model="searchRef" @input="clearSelection" :label="$t('Filter')" flat hide-details
              ...
              <v-list :expand="true" nav dense>
                ...
                <v-list-item v-for="chan in singleChannels" :key="chan.id" @click="toggleSelection(chan.id)"
                  :class="{ 'v-item--active': isSelected(chan.id), 'v-list-item--active': isSelected(chan.id), 'black--text': isSelected(chan.id) }"
                  :multiple="multiselect">
                  <v-list-item-icon>
                    <v-icon>mdi-access-point</v-icon>
                  </v-list-item-icon>
...
export default {
  ...
  setup (props, { emit }) {
    ...
    function filteredChannels () {
      if (!channelsListRef.value) return []
      const arr = channelsListRef.value
      const result = []
      const chanMap = {}
