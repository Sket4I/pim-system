<template>
  <div v-if="attr">
          <div class="d-inline-flex align-center">
            <v-text-field color="black" style="min-width: 100%" v-model="attr.identifier"  :readonly="attr.internalId !== 0" :rules="identifierRules" :label="$t('Config.Attributes.Identifier')" required></v-text-field>
          ...
          <v-select item-color="black" color="black" v-model="attr.type" :items="typeSelection" :rules="typeRules" :label="$t('Config.Attribute.Type')"></v-select>
          ...
          <v-text-field color="black" v-if="attr.type === AttributeType.Text || attr.type === AttributeType.Integer || attr.type === AttributeType.Float || attr.type === AttributeType.Date || attr.type === AttributeType.DateTime" v-model="attr.pattern" :label="$t('Config.Attribute.Pattern')" required></v-text-field>
          ...
          <v-container v-if="attr.type === AttributeType.LOV">
            <v-row v-if="attr.type === AttributeType.LOV">
              <v-autocomplete color="black" v-model="attr.lov" :items="lovSelection" :label="$t('Config.Attribute.LOV')" clearable clear-icon="mdi-close-circle-outline"></v-autocomplete>
          ...
          <v-text-field color="black" v-model="attr.order" type="number" :label="$t('Config.Attributes.Order')" required></v-text-field>
    ...
    <template v-if=changeLOVDialogRef>
      <v-dialog v-model="changeLOVDialogRef" persistent max-width="1200px">
        <v-card>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="black" text v-if="canEditConfig && lov.id !== -1" @click="save">{{ $t('Save') }}</v-btn>
            <v-btn color="black" text @click=closeChangeLOV()>{{ $t('Cancel') }}</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
    </template>
