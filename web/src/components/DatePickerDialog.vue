<template>
  <v-dialog v-model="datePickerDialogRef" persistent width="45%">
    <v-card>
            ...
            <v-col cols="12" class="pa-0 overflow-y-auto" style="max-height: 70vh">
              ...
              <v-tabs-items v-model="tabRef">
                <v-tab-item>
                  ...
                  <v-row>
                    <v-text-field color="black" v-model="datetime" class="ml-5 mr-5"></v-text-field>
                  </v-row>
                </v-tab-item>
                <v-tab-item>
                  <v-container fluid>
                    <v-row>
                      <v-select item-color="black" color="black" class="ml-3 mr-3" :items="items" v-model="perem" :label="$t('Search.Filter.Attribute.Value')"></v-select>
                    </v-row>
                    <v-row>
                      <v-text-field color="black" class="ml-3 mr-3" v-model="numberofdays" type="number" :label="$t('DatePickerDialog.Numberofdays')"></v-text-field>
                    </v-row>
                  </v-container>
                </v-tab-item>
              </v-tabs-items>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="black" text @click="datePickerDialogRef = false">{{ $t('Cancel') }}</v-btn>
