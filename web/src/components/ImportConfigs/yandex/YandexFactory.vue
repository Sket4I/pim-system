          ...
          <template v-slot:default>
                ...
                <th class="text-left grey lighten-3 py-4 px-0" style="width: 50px;">
                  <v-tooltip top>
                    <template v-slot:activator="{ on }">
                      <v-btn v-on="on" class="pa-0" icon color="black" @click="addCategory"><v-icon
            ...
            <tbody>
              <tr v-for="(elem, j) in categories" :key="j">
                <td class="pa-1 pr-10">
                  <v-autocomplete color="black" v-model="elem.attribute" @change="updateMappings" @click:clear="updateMappings"
                ...
                <td class="pa-1 pr-10">
                  <v-autocomplete color="black" v-model="elem.column" @change="updateMappings" @click:clear="updateMappings"
                ...
                <td class="pa-1 pr-10">
                  <v-text-field color="black" v-model="elem.expression" @input="updateMappings" dense class="mt-4 ml-3 mr-3"
          ...
          <template v-slot:default>
            <thead>
              <tr>
                ...
                <th class="text-left grey lighten-3 py-4 px-0" style="width: 50px;">
                  <v-tooltip top>
                    <template v-slot:activator="{ on }">
                      <v-btn v-on="on" class="pa-0" icon color="black" @click="addOffer"><v-icon
                          dark>mdi-plus</v-icon></v-btn>
                    </template>
                    <span>{{ $t('Add') }}</span>
                  </v-tooltip>
            ...
            <tbody>
              <tr v-for="(elem, j) in offers" :key="j">
                ...
                <td class="pa-1 pr-10">
                  <v-autocomplete color="black" v-if="elem.source === 'element'" v-model="elem.column" @change="updateMappings" @click:clear="updateMappings"
                    :items="ymStandardAttributes" item-text="name" item-value="name"
                    :label="$t('ImportConfig.MappingsTable.SelectValue')" clearable></v-autocomplete>
                  <v-text-field color="black" v-else v-model="elem.column" :label="$t('ImportConfig.MappingsTable.SelectValue')" @input="updateMappings" dense class="mt-4 ml-3 mr-3" />
                </td>
                <td>
                  <v-select item-color="black" color="black" v-model="elem.source" item-text="name" item-value="id" :items="sources" :label="$t('ImportConfig.MappingsTable.DataSource')" />
                </td>
                <td class="pa-1 pr-10">
                  <v-text-field color="black" v-model="elem.expression" @input="updateMappings" dense class="mt-4 ml-3 mr-3"
                    append-outer-icon="mdi-message-outline" @click:append-outer="showExpression(elem)" />
